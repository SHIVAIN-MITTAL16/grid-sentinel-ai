...
import { Activity, Globe2, FlaskConical, BrainCircuit, Snowflake, ClipboardCheck } from "lucide-react";
...
const NAV = [
  ...
  { to: "/evidence-room", label: "Evidence Room", icon: ClipboardCheck, code: "06" },
] as const;
  ...
  return (
    <div className="min-h-screen flex flex-col">
            ...
            {NAV.map((n) => {
              ...
              return (
                <Link
                  key={n.to}
                  to={n.to}
                  className={`group relative px-3 py-2 rounded-md text-sm font-medium flex items-center gap-2 transition-colors whitespace-nowrap ${active ? "text-foreground bg-[oklch(0.72_0.18_245/0.12)]" : "text-muted-foreground hover:text-foreground hover:bg-[oklch(0.72_0.18_245/0.06)]"}`}
                >
      ...
      <footer className="border-t border-[oklch(0.72_0.18_245/0.12)] px-6 py-3 text-[10px] font-mono text-muted-foreground flex items-center justify-between">
        <span>GRID SENTINEL AI · v7.4.0 · MODEL TS-49</span>
...
function ClockUTC() {
  ...
  useEffect(() => {
    setT(new Date().toUTCString().slice(17, 25));
